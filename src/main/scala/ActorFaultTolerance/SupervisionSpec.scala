...
class SupervisionSpec extends TestKit(ActorSystem("SuperVisionSpec"))
  ...
  "A supervisior" should {
    "resume its child in case of minor fault" in {
      val supervisior = system.actorOf(Props[Supervisior])
      supervisior ! Props[FussyWordCounter]
  ...
  "restart its child in case of an empty sentence" in {
    val supervisor = system.actorOf(Props[Supervisior])
  ...
  "escalate an error when it doesn't know what to do" in {
    val supervisor = system.actorOf(Props[Supervisior], "supervisor")
    supervisor ! Props[FussyWordCounter]
    val child = expectMsgType[ActorRef]
...
object SupervisionSpec {
  ...
  class NoDeathOnRestartSupervisor extends Supervisior {
    override def preRestart(reason: Throwable, message: Option[Any]): Unit = {
      // empty
    }
  }

  class AllForOneSupervisor extends Supervisior {
  ...
  class Supervisior extends Actor {

    val supervisiorStrategy: SupervisorStrategy = OneForOneStrategy() {
      case _: NullPointerException => Restart
      case _: IllegalArgumentException => Stop
      case _: RuntimeException => Resume
      case _: Exception => Escalate
